...
//******************************************************************************************************************
  ...
  var(finalClusters, clustersInfo) = TensorTucker.transformClusters(tensorRDD, MySpark.sc.parallelize(clusteredRDDmat2.map{ case(x, y) => y}), clusterMembers, tensorInfo, coreRank.clone(), cluNum, 0)
  ...
  val(coreTensors, coreInfo, basisMatrices, iterRecord) = TensorTucker.deComp2 (test2, clustersInfo(2), deCompSeq2, coreRank, maxIter, epsilon )
  ...
  // ComputeApproxNorm( TensorPath, UnfoldPath, CorePath, Basis, CluDim, TensorNorm, DeCompSeq, ...
  //											 CluDeCompSeq, DataClass )
  ...
  // save basis matrices
  //Tensor.saveTensorHeader()


  // for each cluster
  for(c <- cluNum to 1 by -1){
    TensorTucker.computeApprox(test2, clustersInfo(c - 1), cluNum, coreTensors, coreInfo, basisMatrices, deCompSeq, 0)
  }
