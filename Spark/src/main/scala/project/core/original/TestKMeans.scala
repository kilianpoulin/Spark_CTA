...
object TestKMeans {
  ...
  def main(args: Array[String]) {
    ...
    val points = List(Point(1.0, 2.0, 3.0), Point(8.0, 14.0, 3.0), Point(580, 21.0, 3.0), Point(35.0, 26.0, 3.0), Point(18.0, 208.0, 3.0))
  ...
  def createRandomCentroids(points: List[Point]): Map[Point, List[Point]] = {
    ...
    randomIndices.foreach(println)

    points
      .zipWithIndex
      .filter({ case (_, index) => randomIndices.contains(index) })
      .map({ case (point, _) => (point, Nil) }).foreach(println)
