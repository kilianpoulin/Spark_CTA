...
import breeze.linalg.{Axis, DenseMatrix, DenseVector, norm, sum, svd}
...
object TensorTucker {
  ...
  def computeApprox(tensorRDD: RDD[(CM.ArraySeq[Int], DenseMatrix[Double])], tensorInfo: Tensor.TensorInfo, cluNum: Int, coreTensors: RDD[(CM.ArraySeq[Int], DenseMatrix[Double])],
    ...
    var coreTmp = Tensor.TensorUnfoldBlocks( coreTensors.map{ case(x,y) => (x, new DMatrix(y.rows, y.cols, y.valuesIterator.toArray))}, cluDim, coreInfo.tensorRank, coreInfo.blockRank, coreInfo.blockNum).filter{ x => x != null}
    // assemble
    var newmat1 = coreTmp.map{ case(x,y) => y}.filter{ x => x.rows == coreInfo.blockRank(cluDim) }.reduce((a,b) => DenseMatrix.horzcat(a,b))
    var newmat2 = coreTmp.map{ case(x,y) => y}.filter{ x => x.rows == coreInfo.blockFinal(cluDim) }.reduce((a,b) => DenseMatrix.horzcat(a,b))
    var newmat = DenseMatrix.vertcat(newmat1, newmat2)
    ...
    // project each slice on the dual mode basis matrix
    ...
    val unfApprox = Tensor.TensorUnfoldBlocks2(approx.map{ case(x,y) =>
  ...
  def calcNorm(matrix: DenseMatrix[Double]): DenseVector[Double] ={
    var normMatrix: DenseVector[Double] = DenseVector.zeros[Double](matrix.rows)
    val mat = matrix.t
    for(i <- 0 to matrix.rows - 1){
      normMatrix(i) = norm(mat(::, i))
    }
    normMatrix
  }
