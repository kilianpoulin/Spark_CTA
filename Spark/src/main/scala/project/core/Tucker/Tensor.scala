...
import breeze.linalg.{DenseMatrix => BMatrix, DenseVector, eigSym, min}
import breeze.numerics.{ceil, sqrt}
import org.apache.hadoop.io.{BytesWritable, LongWritable}
import org.apache.hadoop.mapred.{FileInputFormat, JobConf}
import org.apache.spark.broadcast.Broadcast
import org.apache.spark.rdd._
import org.apache.spark.{Partitioner, SparkContext}
import project.core.javacode.{BinaryOutputFormat, MyFixedLengthInputFormat}
...
object Tensor
{
  ...
  // Save tensor block
  //-----------------------------------------------------------------------------------------------------------------
  def saveTensorBlock ( tensorPath: String, tensorRDD: RDD[ (CM.ArraySeq[Int], BMatrix[Double]) ] ) =
  {
    // Set Hadoop configuration
    val hadoopJobConf = new JobConf( MySpark.sc.hadoopConfiguration )
  ...
  // Save Basis matrices
  //-----------------------------------------------------------------------------------------------------------------
  def saveBasisMatrices ( basisPath: String, bcBasisMatrixArray: Array[ Broadcast[BMatrix[Double]] ],
                          dimSeq: Array[Int] ) =
  {
    // Set Hadoop configuration
    val hadoopJobConf = new JobConf( MySpark.sc.hadoopConfiguration )
    ...
    val matrixArray = new Array[ ( Int, BMatrix[Double] ) ]( matrixNum )
    ...
    for( i <- 0 until matrixNum )
    {
      if( dimSeq.contains( i ) )
        ...
        matrixArray( i ) = ( 1, BMatrix.fill(1,1){0.0} )
    }
  ...
  //-----------------------------------------------------------------------------------------------------------------
  def modeNProducts( tensorRDD: RDD[ (CM.ArraySeq[Int], BMatrix[Double]) ], tensorInfo: TensorInfo,
                     prodsSeq: Array[Int], bcMatrixArray: Array[Broadcast[BMatrix[Double]]] )
  : ( RDD[ (CM.ArraySeq[Int], BMatrix[Double]) ], TensorInfo )  =
  {
    ...
    @tailrec
    def recursiveCall( tensorRDD: RDD[ (CM.ArraySeq[Int], BMatrix[Double]) ], tensorInfo: TensorInfo, iter: Int )
    : ( RDD[ (CM.ArraySeq[Int], BMatrix[Double]) ], TensorInfo )  =
    {
      if( iter == prodsDims )
      {
  ...
  // Perform mode-N products
  //-----------------------------------------------------------------------------------------------------------------
  def modeNProducts2( tensorRDD: RDD[ (CM.ArraySeq[Int], BMatrix[Double]) ], tensorInfo: TensorInfo,
                      prodsSeq: Array[Int], bcMatrixArray: Array[Broadcast[BMatrix[Double]]] )
  : ( RDD[ (CM.ArraySeq[Int], BMatrix[Double]) ], TensorInfo )  =
  {
  ...
  //-----------------------------------------------------------------------------------------------------------------
  def modeNProduct( tensorRDD: RDD[ (CM.ArraySeq[Int], BMatrix[Double]) ], tensorInfo: TensorInfo, prodDim: Int,
                    bcMatrix: Broadcast[BMatrix[Double]] )
  :( RDD[ (CM.ArraySeq[Int], BMatrix[Double]) ], TensorInfo )  =
  {
  ...
  //-----------------------------------------------------------------------------------------------------------------
  def extractBasis( tensorRDD: RDD[ (CM.ArraySeq[Int], BMatrix[Double]) ], tensorInfo: TensorInfo, extDim: Int,
                    extRank: Int )
  : BMatrix[Double] =
  {
    ...
    var basisMatrix = BMatrix.zeros[Double]( covMatrix.rows, extRank)
    ...
    if(covMatrix.rows < extRank){
      ...
      //add missing columns
      //basisMatrix = DenseMatrix.horzcat(basisMatrix, BMatrix.zeros[Double](covMatrix.rows, extRank - covMatrix.cols))
  ...
  //-----------------------------------------------------------------------------------------------------------------
  def getCovMatrix( tensorRDD: RDD[ (CM.ArraySeq[Int], BMatrix[Double]) ], tensorInfo: TensorInfo, extDim: Int )
  : BMatrix[Double] =
  {
    // Reset key and group
  ...
  // Project onto matrix
  //-----------------------------------------------------------------------------------------------------------------
  def projectOntoMatrix(matrixA: BMatrix[Double], matrixB: BMatrix[Double], TransFlagA: Boolean, TransFlagB: Boolean): BMatrix[Double] ={

    var transMatA = matrixA
    var transMatB = matrixB
    var transSize = matrixB.rows
    var projectionMat: BMatrix[Double] = BMatrix.zeros[Double](transSize, transSize)
  ...
  // Project onto matrix
  //-----------------------------------------------------------------------------------------------------------------
  def projectOntoMatrix(matrixA: Array[BMatrix[Double]], matAInfo: Tensor.TensorInfo, matrixB: BMatrix[Double], TransFlagA: Boolean, TransFlagB: Boolean): BMatrix[Double] ={
  ...
  //-----------------------------------------------------------------------------------------------------------------
  private def convertMatrix2Bytes( dim: Int, matrix: BMatrix[Double] ): ( BytesWritable, BytesWritable ) =
  {
  ...
  //-----------------------------------------------------------------------------------------------------------------
  def convertTuple2Bytes( blockSubIndex: CM.ArraySeq[Int], tensorMatrix: BMatrix[Double] )
  ...
  // Convert tuple2( blockSubIndex, DenseVector ) tp bytes array
  //-----------------------------------------------------------------------------------------------------------------
  def convertTuple2Bytes( tensorMatrix: BMatrix[Double] )
  : ( BytesWritable, BytesWritable ) =
  {
  ...
  def blockTensorAsVectors(m: BMatrix[Double]): Array[Vector[Double]] ={
  ...
  // Perform local tensor unfold
  //-----------------------------------------------------------------------------------------------------------------
  def localTensorUnfold( tensorMatrix: BMatrix[Double], unfoldDim: Int, tensorRank: Array[Int] )
  : BMatrix[Double] =
  {
    val tensorDims = tensorRank.length
    var unfoldMatrix: BMatrix[Double] = null
  ...
  def localTensorRefold( tensorMatrix: BMatrix[Double], refoldDim: Int, tensorRank: Array[Int] )
  : BMatrix[Double] =
  {
    var refoldMatrix: BMatrix[Double] = null
    val tensorDims = tensorRank.length
  ...
  //-----------------------------------------------------------------------------------------------------------------
  private def localMultiDimMMMany( blockSubIndex: CM.ArraySeq[Int], denseMatrix: BMatrix[Double],
                                   prodsSeq: Array[Int], bcMatrixArray: Array[Broadcast[BMatrix[Double]]],
                                   blockRank: Array[Int], blockNum: Array[Int], blockFinal: Array[Int],
                                   prodsTensorRank: Array[Int], prodsBlockNum: Array[Int] )
  : CM.ArraySeq[( CM.ArraySeq[Int], BMatrix[Double] )] =
  {
    ...
    val result = new CM.ArraySeq[ (CM.ArraySeq[Int], BMatrix[Double]) ]( prodsBlockNum.product )
  ...
  //-----------------------------------------------------------------------------------------------------------------
  def localMMMany( blockSubIndex: CM.ArraySeq[Int], denseMatrix: BMatrix[Double], prodDim: Int,
                           bcProdMatrix: Broadcast[BMatrix[Double]], blockRank: Array[Int], blockNum: Array[Int],
  ...
  val prodDimRank = bcProdMatrix.value.cols
    ...
    val result = new CM.ArraySeq[ (CM.ArraySeq[Int], BMatrix[Double]) ]( maxProdBlockIndex + 1 )
    var colIndex = 1 to 2
    ...
    for( index <- 0 to maxProdBlockIndex )
    {
      ...
      var prodMatrix: BMatrix[Double] = null
  ...
  //-----------------------------------------------------------------------------------------------------------------
  private def extractBasisResetKey( blockSubIndex: CM.ArraySeq[Int], tensorMatrix: BMatrix[Double], dstDim: Int )
  : ( CM.ArraySeq[Int], ( Int, BMatrix[Double] ) ) =
  {
    // Generate new key
  ...
  private def computePartialCovMatrix( generalSubIndex: CM.ArraySeq[Int],
                                       blocks: Iterable[ (Int, BMatrix[Double]) ], tensorInfo: TensorInfo,
                                       dstDim: Int )
  : BMatrix[Double] =
  {
    // Set variable
    ...
    val covMatrix = BMatrix.zeros[Double]( tensorInfo.tensorRank(dstDim), tensorInfo.tensorRank(dstDim) )
  ...
  def matrixToRDD(m: BMatrix[Double]): Array[Vector[Double]]  = {
    val columns = m.toArray.grouped(m.rows)
    val rows = columns.toSeq.transpose
