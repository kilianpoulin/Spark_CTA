...
public class MyMapRedFixedLengthRecordReader implements RecordReader<LongWritable, BytesWritable>
{
    private int recordLength;
    private int splitSize; // My extra content
    // Make use of the new API implementation to avoid code duplication.
    private MyMapReduceFixedLengthRecordReader reader;

    public MyMapRedFixedLengthRecordReader( Configuration job, FileSplit split, int recordLength ) throws IOException
    {
        this.recordLength = recordLength;
        reader = new MyMapReduceFixedLengthRecordReader( recordLength );
        reader.initialize( job, split.getStart(), split.getLength(), split.getPath() );

        // My extra content
        splitSize = (int)reader.getSplitSize();
        if( splitSize < this.recordLength )
            this.recordLength = splitSize;
    }
